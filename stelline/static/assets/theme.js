/* 다크 모드 - 서버가 그리는 화면(관리자·로그인) 전용.
 *
 * 공개 화면은 React 앱이 같은 저장소 키(stelline.theme)를 쓰는 ThemeProvider 로
 * 이어받는다. 그래서 어느 쪽에서 골라도 다른 쪽에 그대로 반영된다.
 *
 * 이 화면들의 <head>에서 먼저 실행해 화면이 그려지기 전에 테마를 정한다.
